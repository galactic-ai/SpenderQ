...
def similarity_restframe(instrument, model, s=None, slope=1.0,
        individual=False, wid=5, bound=[1000, 2100]): #bound=[4000,7000]):
    _, s_size = s.shape
    device = s.device
...
if __name__ == "__main__":
    ...
    train(models, instruments, trainloaders, validloaders, n_epoch=n_epoch,
          n_batch=args.batch_number, lr=args.rate, aug_fcts=[None], 
          similarity=args.similarity, consistency=args.consistency, 
          outfile=args.outfile, losses=losses, verbose=args.verbose)
